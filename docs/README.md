...
- [x] 이름 정보로부터 크루를 생성한다.
- [ ] 크루를 매칭한다.
  - [x] 페어 목록을 섞어서 앞에서부터 매칭한다.
  - [x] 홀수인 경우 마지막 남은 크루를 마지막 페어에 포함한다.
  - [x] 같은 레벨에서 이미 페어로 만난 경우, 다시 매칭하고 3회까지 매칭이 되지 않거나 매칭을 할 수 없는 경우 에러 메시지를 출력한다.
    - [ ] 페어의 크루가 3명인 경우를 처리한다.
    - 백엔드, 미션1, 자동차경주에서 크루원 1과 크루원 2가 페어였다면 백엔드, 미션1의 다른 미션에서는 크루원 1과 2가 페어가 될 수 없음
- [x] 매칭 정보를 초기화한다. 
...
- [x] 사용자로부터 매칭하고자하는 과정, 레벨, 미션을 입력받는다.
  ...
  - [x] 없는 과정일 경우 예외처리한다.
  - [x] 이미 매칭된 정보가 있는 경우, 예. 아니오를 입력받고 처리한다.

페어 조회
- [x] 사용자로부터 매칭하고자하는 과정, 레벨, 미션을 입력받는다.
  - [x] 매칭 정보가 없는 경우 매칭 이력이 없다고 출력한다.
- [x] 매칭 결과를 출력한다.
