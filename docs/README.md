...
- 선택할 기능을 입력받는다
  ...
  - 페어 초기화(3)
  - 종료(Q)
  - 이외의 값이 입력된다면 에러메시지를 출력하고 다시 입력하게 한다.
- 페어 매칭을 한다
  - 크루의 목록을 순서대로 섞어 앞에서부터 두명씩 페어를 맺는다.
  - 미션을 함께 수행할 페어를 두명씩 매칭한다.
  - 페어 매칭 대상이 홀수인 경우 한 페어는 3인으로 구성한다. 
  - 같은 레벨에서 이미 페어를 맺은 크루와는 다시 페어로 매칭될 수 없다. 만약 다시 페어로 매칭 됐다면 순서를 다시 섞어 매칭한다.
  - 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 에러 메시지를 출력한다.
  - 이미 매칭된 경우를 매칭하려고 하면 다시 매칭 할지 물어보고 '예'라면 다시 매칭, '아니오'라면 패스한다.

- 매칭된 페어를 조회한다
  - 매칭이 되지 않는 경우를 조회하면 에러 메시지를 출력하고 조회 할 매칭을 다시 입력하게 한다.

- 매칭과 매칭 조회 기능을 선택하면 과정과 미션을 출력하고 매칭하고자 하는 과정, 레벨, 미션을 입력 받는다.
  - 잘못된 과정, 레벨, 미션을 입력하면 에러메시지를 출력하고 다시 입력하게 한다.

- 페어 초기화 기능을 선택하면 모든 매칭 기록을 없앤다.

- 종료 기능을 선택하면 프로그램을 종료시킨다.
