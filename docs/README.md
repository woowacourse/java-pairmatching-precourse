...
- [X] 프론트 크루들의 파일을 로드한다.
- [X] 사용자에게 기능을 입력받는다.
    - [X] 기능은 1. 페어 매칭, 2. 페어 조회 3. 페어 초기화, Q. 종료로 이루어진다.
    - [X] 이 외의 기능을 입력한 경우 에러 메시지 출력 후 재입력 받는다.

### 페어 매칭

- [X] 사용자에게 과정, 미션을 출력한다.
- [X] 사용자에게 페어를 매칭할 과정, 레벨, 미션을 콤마로 구분받아 입력받는다.
    - [X] 없는 과정, 레벨, 미션을 입력한 경우 에러 메시지 출력 후 재입력받는다.
    - [X] 레벨에 맞지 않는 미션 입력 시 에러 메시지 출력 후 재입력 받는다.
    - [X] 콤마로 구분되지 않거나, 3의 데이터를 입력하지 않은 경우 에러 메시지 출력 후 재입력받는다.
- [X] 기존 매칭 정보가 있는지 확인.
    - [X] 매칭 정보가 있는 경우 다시 매칭할 것인지 확인
    - [X] `아니오`를 선택할 경우 다시 과정, 레벨, 미션 입력받음.
    - [X] `네`를 선택할 경우 매칭 진행
    - [X] 다른 값을 입력한 경우 에러 발생 후 재입력
- [X] 미션을 수행할 페어를 두명씩 매칭한다.
    - [X] 랜덤으로 섞인 페어 목록에서 페어 매칭을 할 때 앞에서부터 두명씩 페어를 맺는다.
- [ ] 매칭 결과 출력
- [X] 매칭 대상이 홀수 인 경우 한 페어는 3인으로 구성된다.
    - [X] 홀수인 경우 마지막 남은 크루는 마지막 페어에 포함시킨다.
- [ ] 같은 레벨에서 이미 페어를 맺은 크루와는 다시 페어로 매칭될 수 없다.
    - [ ] 3회까지 매칭을 시도한다.
    - [ ] 3회까지 시도 후에도 매칭을 할 수 없다면, 에러 메시지를 출력한다.
- [ ] 재매칭 시 안내 문구를 출력 후 매칭을 진행한다.
- [ ] 재매칭 시 아니요를 선택할 경우 코스, 레벨, 미션을 다시 선택한다.

### 페어 조회

- [X] 사용자에게 과정, 미션을 출력한다.
- [X] 사용자에게 과정, 레벨, 미션을 콤마로 구분 받아 입력받는다.
    - [X] 없는 과정, 레벨, 미션을 입력한 경우 에러 메시지 출력 후 재입력받는다.
    - [X] 레벨에 맞지 않는 미션 입력 시 에러 메시지 출력 후 재입력 받는다.
    - [X] 콤마로 구분되지 않거나, 3의 데이터를 입력하지 않은 경우 에러 메시지 출력
- [X] 매칭 결과 출력
- [X] 매칭이력이 없으면 없다고 안내한다.

### 페어 초기화

- [X] 지금까지 설정된 모든 페어 목록을 초기화한다.
