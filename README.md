...
- [ ] 시작시, 기능의 종류 목록을 출력하는 기능
- [ ] 기능의 종류를 선택하는 기능
    ```
    [예외]
    - 기능의 종류가 존재하지 않는 기능인 경우 ( 현존하는 기능 : 1, 2, 3, Q )
    ``` 
...
- [ ] 과정과 미션을 출력하는 기능
- [ ] 과정, 레벨, 미션을 입력 받아 페어를 매칭하는 기능
    ```
    [예외]
    - 쉼표(,)로 구분하여 과정, 레벨, 미션 단위로 나누어지지 않으면 예외처리
    - 과정, 레벨, 미션의 각 항목이 존재하지 않으면 예외처리
    ```

- [ ] 페어가 매치된 결과를 출력하는 기능
    #### 매칭 정보가 있을 경우
  - [ ] 매칭 재시도를 묻는 문구를 출력하는 기능
  - [ ] 매칭 재시도 여부를 입력 받는 기능
      ```
      [예외]
      - 존재하는 명령이 아니면 예외처리 ( 현존하는 명령 : 네, 아니오 )
      ```
