# 기능 목록
- [x] 프로그램 시작 전, 레벨에 맞는 미션을 초기화 한다.
- [x] 사용자는 페어 매칭, 페어 조회, 페어 초기화, 종료의 기능을 선택할 수 있다.
  - [x] 해당 기능 외의 입력을 받으면 에러 메시지를 띄운다.
- [x] 페어 매칭을 선택하면 매칭할 과정, 레벨, 미션을 입력할 수 있다.
  - [x] 과정, 레벨, 미션은 이미 정해진 값 내에서 입력해야 한다.
  - [x] 각 입력값은 ','로 구분되어 있다.
- [x] 백앤드 혹은 프론트앤드 인원을 자바 입출력으로 불러온다.
- [x] 이름 목록을 랜덤으로 섞는다.
- [x] 앞에서 부터 2명씩 묶은 후 인원이 홀수여서 1명이 남는다면 마지막 팀에 포함 시킨다.
- [x] 같은 레벨에서 이미 페어로 만난 적이 있는 크루끼리 매칭되면 다시 진행한다. (이전 결과와 비교)
- [x] 3회 시도까지 매칭이 되지 않거나 경우의 수가 없으면 에러를 출력한다.
- [x] 입력 받는 과정, 레벨, 미션이 있는지 확인한다.
- [x] 이미 해당 과정, 레벨, 미션이 있는데 다시 매칭하고자 하면 매칭 정보가 존재한다는 문구를 띄우고 재매칭 여부를 입력 받는다.
- [x] 기존 매칭결과를 재매칭 결과로 대체한다.
- [x] 페어 조회를 선택하면 조회할 과정, 레벨, 미션을 입력할 수 있다.
- [x] 해당 입력으로 조회된 페어가 없는 경우 에러 메시지를 띄운다.
- [x] 해당 입력으로 조회된 페어가 있으면 제일 최근에 매칭된 페어를 띄운다.
- [x] 페어 초기화를 선택하면 모든 페어를 초기화 한다.
