...
- 기능 선택하기
  - 입력받기 전 "기능을 선택하세요.\n1. 페어 매칭\n2. 페어 조회\n3. 페어 초기화\nQ. 종료"를 출력한다.
  - 사용자가 입력하는 값은 주어진 선택지 중 하나여야 한다.
  - 사용자가 입력하는 값은 '1'처럼 번호일 수 있고, '페어 매칭'처럼 기능의 이름일 수 있고, '1. 페어 매칭'처럼 번호와 기능일 수 있다.
  - ⚠　잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고 `[ERROR]`로 시작하는 에러 메시지를 출력한 후 해당 부분부터 다시 입력받는다.
- 사전 제공된 정보 출력하기
  - 과정과 레벨은 제공된 힌트(enum)를 사용한다. https://github.com/qbang/java-pairmatching-precourse#%EF%B8%8F-%ED%9E%8C%ED%8A%B8
- 과정, 레벨, 미션 입력받기
  - 입력받기 전 "과정, 레벨, 미션을 선택하세요.\n ex) 백엔드, 레벨1, 자동차경주\n"라는 메시지를 출력한다. 
  - ⚠　각 인자는 사전에 제공된 정보에 포함되어야 한다.
  - ⚠　각 인자는 쉼표로 구분되어야 한다.
  - ⚠　올바른 인자 개수의 합은 3이어야 한다.
  - ⚠　잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고 `[ERROR]`로 시작하는 에러 메시지를 출력한 후 해당 부분부터 다시 입력받는다.
- 크루들의 이름 불러오기
  - `src/main/resources/backend-crew.md`와 `src/main/resources/frontend-crew.md`에서 이름을 불러온다.
  - ⚠　크루의 이름은 중복될 수 없다.
- 페어 두명씩 매칭하기
  - 섞는 것은 이름 목록을 List<String>으로 준비한 뒤, `camp.nextstep.edu.missionutils.Randoms`의 shuffle 메서드를 활용한다.
  - 섞인 목록에서 앞에서부터 순서대로 두명씩 페어를 맺는다.
  - 만약 매칭 대상이 홀수라면 마지막 페어는 3인으로 구성한다.
  - 같은 레벨 동안은 같은 페어를 만나지 않는다.
  - ⚠　3회 시도까지 매칭이 실패한다면 에러 메시지를 출력한다.
- 페어 매칭/재매칭 결과 출력하기
  - 매칭이 정상적으로 수행되면 한꺼번에 결과를 출력한다.
  - 출력하기 전 "페어 매칭 결과입니다."라는 안내 메시지를 출력한다.
  - 매칭 결과는 `이름 : 이름`으로 출력하고, 각 매칭 결과 간 줄을 바꿔준다.
  - 출력 후 사용자는 기능을 선택할 수 있다.
- 페어 재매칭하기
  - 매칭 정보가 있을 때 입력받기 전 "매칭 정보가 있습니다. 다시 매칭하시겠습니까?"라는 메시지를 출력한다.
  - `아니오`를 선택하면 과정, 레벨, 미션을 다시 입력받는다.
  - 같은 레벨 동안은 매칭되었던 페어를 다시 만나지 않는다.
  - `예`를 선택하면 다시 매칭한다.
  - 3회 시도까지 매칭이 실패한다면 에러 메시지를 출력한다.
- 페어 조회하기
  - 입력받기 전 과정과 미션 정보를 출력한다.
  - 과정, 레벨, 미션을 입력받는다.
  - ⚠　각 인자는 사전에 제공된 정보에 포함되어야 한다.
  - ⚠　각 인자는 쉼표로 구분되어야 한다.
  - ⚠　올바른 인자 개수의 합은 3이어야 한다.
  - ⚠　잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고 `[ERROR]`로 시작하는 에러 메시지를 출력한 후 해당 부분부터 다시 입력받는다.
  - 조회 후 사용자는 기능을 선택할 수 있다.
- 페어 초기화하기
  - "초기화 되었습니다."라는 메시지를 출력한다.
  - 초기화하면 이전에 매칭되었던 기록들을 모두 삭제한다.
  - 초기화 후 사용자는 기능을 고를 수 있다.
