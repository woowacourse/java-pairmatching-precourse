...
3. 기능 선택
 - ```에러``` 존재하지 않는 기능 선택시 에러 발생

4. 페어 매칭
  - 과정, 레벨, 미션을 정상적으로 입력했는지 확인
      - ```에러``` , 기준으로 분할 시 제대로 분리되지 않는 경우
      - ```에러``` 빈칸으로 입력되는 경우
      - ```에러``` 존재하지 않는 기능을 선택한 경우
  - ```repository```를 확인하고, 이미 존재하면 ```페어 재매칭``` 기능 수행
     - ```에러``` 입력을 네, 아니오로 하지 않은 경우.
  - ```Shuffle```을 이용해서 섞기.
  - 짝수인가? 그대로 조편성, 홀수인가? 마지막 3명 한 조로 분류
  - 조 편성 과정에서 레벨 별 ```repository```를 확인하고, 만약 겹치는 그룹이 있을 경우 다시 1번으로 복귀 (총 3회)
  - ```에러``` 3회 이상 조편성 실패시 에러 발생
