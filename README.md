...
- 크루 이름 파일을 읽어 리스트에 넣기
- 기능 선택 입력 받기
- 페어 매칭(1)
    - 과정, 미션을 출력한다.
    - 과정, 레벨, 미션을 입력 받는다.
        - 컴마(,)로 구분한다.
    - 이름 목록을 shuffle 하여 페어 매칭하기
        - 앞에서 부터 2명씩 페어를 맺고, 이름이 홀수 개이면 마지막은 3명으로 묶는다.
        - 같은 레벨에서 이미 만난 적 있는 페어가 존재할시 처음부터 다시 한다.
        - 시도는 3회까지 할 수 있고, 경우의 수가 없으면 에러 메세지를 출력한다.
    - 재시도를 할 시에 다시 매칭할것인지 입력을 받는다.
        - 네 -> 같은 과정, 레벨, 미션으로 매칭한다.
        - 아니오 -> 과정, 레벨, 미션을 다시 입력 받는다.
- 페어 조회(2)
    - 저장한 레벨의 페어 목록을 출력한다.
- 페어 종료(3)
    - 프로그램을 종료한다.
