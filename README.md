...
### 기능 선택
 - [x] 프로그램의 기능 종류를 출력 하고 그 중 하나를 입력 받는다.
   - [x] 입력이 기능 목록에 포함되는지 검증한다.
   - [x] 검증에 실패하면 `[ERROR] 제공된 기능 중 하나를 선택 하세요`을 출력하고 다시 입력 받는다.
   - [x] 입력을 도메인 type으로 Controller에 반환한다.

### 페어 매칭
 - [x] 우테코에서 제공되는 과정, 레벨, 미션을 출력한다.
 - [x] 이 셋을 입력 받는다.
   - [x] 각 정보는 `,[공백]`으로 구분된다.
   - [x] 과정, 레벨, 미션 3개의 정보가 입력되었는지 검증한다.
   - [x] 검증에 실패하면 `[ERROR] 과정, 레벨, 미션을 입력하세요`를 출력하고 다시 입력 받는다.
   - [x] 입력된 각 정보들이 우테코에서 제공되고 있는 선택 가능한 옵션인지 검증한다.
   - [x] 검증에 실패하면 `[ERROR] 우테코에서 제공하고 있지 않은 옵션입니다`를 출력하고 다시 입력 받는다.
   - [x] 입력을 도메인 type으로 Controller에 반환한다.
 - [x] 이미 매칭 정보가 있을 경우 재매칭 여부를 물은뒤 입력 받는다.
   - [ ] `아니오`를 입력 받은 경우 페어 매칭을 두 번째 과정부터 반복한다.
   - [x] `예`를 입력 받은 경우 이전 매칭 결과를 삭제한다.
   - [x] 그 외의 입력을 받은 경우 `[ERROR] 예 혹은 아니오를 입력하세요`를 출력하고 다시 입력 받는다.
 - [ ] 크루들의 이름 목록을 List<String> 형태로 준비하고 랜덤으로 섞는다.
 - [ ] 앞에서부터 순서대로 두명씩 페어를 맺고, 홀수인 경우 마지막 남은 크루는 마지막 페어에 포함시킨다.
 - [ ] 같은 레벨에서 이미 페어로 만난적이 있는 크루끼리 다시 페어로 매칭 된다면 크루 목록의 순서를 다시 랜덤으로 섞어서 매칭을 시도한다.
 - [ ] 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 `[ERROR] 페어 매칭에 실패하였습니다`를 출력한다.
 - [ ] 패어 매칭에 성공하면 매칭 결과를 저장하고 매칭 결과를 출력한다.
 - [ ] 기능 선택 과정으로 돌아간다.

### 페어 조회
- [ ] 페어 매칭 기능의 첫 번쨰 입력 과정과 동일하게 과정, 레벨, 미션을 입력 받는다.
- [ ] 해당 과정, 레벨, 미션에 매칭 이력이 없으면 `[ERROR] 매칭 이력이 없습니다`를 출력한다.
- [ ] 해당 과정, 레벨, 미션에 매칭 이력이 있으면 저장된 매칭 결과를 출력한다.
- [ ] 기능 선택 과정으로 돌아간다.


### 페어 초기화
- [ ] `초기화 되었습니다`를 출력하고 저장되어 있는 페어 매칭 이력을 삭제한다.
- [ ] 기능 선택 과정으로 돌아간다.

<br/>

-[x] 종료 기능을 선택하면 프로그램을 종료한다.
