...
- [X] 선택 가능한 기능을 띄워주며 기능을 입력 받는다.
  - [X] 입력에 대한 예외처리
- [X] 기능 선택 시 기본 정보를 띄운 후 과정, 레벨, 미션을 입력받는다.
  - [ ] 입력에 대한 예외처리
- [X] Q 입력 시 프로그램을 종료한다.

### 페어 매칭

- [ ] 기본 정보를 띄운 후 과정, 레벨, 미션을 입력 받는다.
- [ ] 랜덤하게 페어 매칭한다.
  - [ ] 홀수인 경우 마지막 남은 크루는 마지막 페어에 포함한다.
  - [ ] 같은 레벨에서 이미 페어로 만난적 있는 크루끼리 매칭된다면, 크루 목록 순서를 다시 랜덤으로 섞는다.

- [ ] 매칭이 된 과정, 레벨, 미션을 선택한 경우 다시 매칭할지 여부를 선택하게 한다.
  - [ ] '네'를 입력한 경우 다시 매칭 시킨다.
  - [ ] '아니오'를 입력한 경우 과정, 레벨, 미션을 다시 입력 받는다.

### 페어 조회

- [ ] 기본 정보를 띄운 후 과정, 레벨, 미션을 입력 받는다.
- [ ] 해당하는 매칭 결과를 띄워준다.
- [ ] 해당하는 매칭이 없다면 에러 헤더와 함께 매칭 이력이 없다고 안내한다.
