...
## 나의 기능구현
- **공통**
  - **ValidatorMessage** 클래스
    - 검증 메시지를 다루는 클래스.
  - **PairMatching** 클래스
    - 페어 매칭 시스템을 총괄하는 클래스.
  - **InputView** 클래스
    - 각 도메인과 `PairMatching`클래스에 존재한다.
    - 사용자의 입력을 담당한다.
    - 기능 선택, 과정과 미션을 묻는 메소드는 `PairMatching`의 `OutputView`클래스 내에 포함한다.
  - **OutputView** 클래스
    - 각 도메인과 `PairMatching`클래스에 존재한다.
    - 사용자의 입력을 담당한다.
    - 과정과 미션을 보여주는 메소드는 `PairMatching`의 `InputView`클래스 내에 포함한다.
  

- **matching 패키지 내 클래스 목록**
  - 크루명이 포함된 **파일 입력**을 받는 `MatchingFileInput`클래스를 구현한다.
  - 매칭 도메인에 필요한 간단한 로직을 `Matching` 클래스에서 구현한다.  
    - 각 레벨 당 보통 3개의 미션이 존재함을 이용한다.
    - 미션명 추가는 `HashMap`자료구조를 이용한다. 미션명을 상수화할 때 생산성이 나아질 경우 상수화를 진행한다.
  - `Matching`클래스와 소통하며 복잡한 로직을 담당하는 `MatchingService` 클래스를 구현한다.
    - 페어 매칭, 페어 조회, 페어 초기화 로직을 맡는다.
    - 매칭 정렬 방법은 `shuffle` 메소드를 이용한다.
    - 매칭 정보가 이미 존재하는지의 정보를 구한다.
  - `Controller` 클래스에서 `View`, `Service` 클래스와 소통한다.
    - 예외 체크 로직이 비교적 간단하므로 예외를 던지는 역할 또한 `Controller` 클래스에서 맡는다.
