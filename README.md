...
- 크루 정보는 src/resources 하위에 md파일로 제공, 파일 입출력으로 읽는다.
- 셔플을 돌려서 2명씩 페어를 지정한다.
    - 홀수일 경우 한 페어는 3명: 남은 한명은 마지막 페어에 포함
    - 예외: 이미 페어했던 크루랑은 매칭 x
    - 예외: 3회 시도까지 매칭이 되지 않으면 에러 메시지
    - 예외: 매칭을 할 수 있는 경우의 수가 없으면 에러 메시지
    - 예외: 입력이 잘못된 경우 IllegalException을 발생시키고 재입력 받는다.
- 각 이름당 매칭 이력 저장
- 과정, 레벨, 미션을 선택하면 해당 미션의 페어 정보 출력
    - 예외: 매칭 이력이 없으면 이력이 없다고 안내
    - 매칭 이력이 있다면, 페어 재매칭 시도(아니오 -> 코스,레벨,미션 다시 선택)
</br>
