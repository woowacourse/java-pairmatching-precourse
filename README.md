...
- 파일 입출력
  - 크루의 이름을 md파일에서 읽어온다.(o)
  - List<Crew> crew의 형태로 만든다.


0. 초기 기능 선택 input 뷰 생성 (o)

1. 페어매칭 2. 페어조회 3. 페어초기화 Q. 종료를 출력하는 뷰 생성 (o)
- 위의 것 중 하나를 입력받는다.  (o)
  - 예외)
  - 1, 2, 3, Q 이외의 다른 단계를 입력한다 (o)
  - 아예 빈칸으로 입력했다.
    
1. 페어매칭
  - 현재 코스가 가지고 있는 과정, 레벨, 레벨별 미션을 출력한다 (o)
  - 과정, 레벨, 미션을 입력받는다. (o)
    - 예외)
    - 존재하지 않는 과정을 입력한다 ex) 백프론트, 백...
    - 존재하지 않는 레벨을 입력한다 ex) 레벨6
    - 레벨 별 존재하지 않는 미션을 입력한다 ex) 레벨5 - 성능개선
    - 과정, 레벨, 미션 중 하나를 빼먹었다.(split이 제대로 되지 않았다) (o)
    - 아예 빈칸으로 입력했다.

   1-1. 매칭을 수행한다

  - 매칭 수행시 조건
    - 이미 저장된 매칭 정보가 있다면 매칭을 다시 수행할건지 입력받는다.
    - "아니오"를 입력한다면 다시 과정-레벨-미션을 입력받는다.

    매칭 수행 방법
    - 1- 크루들의 이름 목록을 준비한다.
    - 2- 크루의 이름 목록 순서를 랜덤으로 섞는다. → shuffle 메소드를 사용한다.
    - 3- 앞에서 순서대로 두명씩 페어를 만든다
    - 4- 이미 페어로 만난적이 있는 크루가 페어가 된다면 2번 과정으로 돌아간다
    - 5- 3회 시도까지 매칭되지 않거나 매칭할 수 있는 경우의 수가 없다면 에러메세지를 출력한다.
    - 재매칭의 경우 기존의 값을 초기화한 후 진행한다. (Crew가 갖고있는 level-crew 맵에서 이전의 값을 삭제한다)
  - 페어매칭 결과를 어딘가에 저장한다.

  - 페어 매칭 수행 결과를 출력한다
    - 페어의 순서는 Shuffle 메서드를 이용한다.
      - 3인 크루는 가장 마지막에 나와야한다.
2. 페어 조회
  - 현재 코스가 가지고 있는 과정, 레벨, 레벨별 미션을 출력한다.(1과 동일)
  - 과정, 레벨, 미션을 입력받는다(1과 동일)
  - 매칭한 페어들의 결과를 보여준다
    - 예외) 매칭된 페어값이 없다

3. 페어 초기화
  - 모든 미션이 가지고있는 페어 매칭결과를 삭제한다.
    - 미션이 갖고 있는 MatchingPairs(List<Pair> 을 삭제한다)
    - Crew 내에 있는 Level-List<Crew> 맵을 삭제한다

생각해야할 포인트
    
- 내가 같은 레벨에서 미션을 같이 했던 크루임을 어떻게 아는가.
  - 크루에 Map을 두고, 내가 같은 레벨에서 미션을 한 크루를 맵에 저장한다.
- MatchingPairs에 3인짝은 한팀만 있어야한다.
