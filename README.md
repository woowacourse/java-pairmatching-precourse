...
- 올바른 입력인지 검증하고 입력이 `Q`일 경우에 프로그램을 종료시킨다.


학생 정보 저장
   - 파일에 있는 학생들을 List 형식으로 저장한다.

페어 매칭
   - 랜덤을 통해 학생 정렬
   - 앞에서부터 두명씩 매칭
   - 홀수인 경우 마지막 페어에는 세명이 매칭
   - 같은 레벨에서 이미 페어로 만난 경우에는 페어 매칭 재시도
     - 아니오를 선택할 경우 코스, 레벨, 미션을 다시 선택
   - 3회 시도까지 매칭이 되지 않거나 매칭할 수 있는 경우의 수가 없으면 에러 메세지 출력

페어 조회 기능

- 입력한 과정, 레벨, 미션과 동일한 페어 매칭이 있는 경우 페어 매칭 결과를 출력
