...
## 🎫 구현할 기능 목록
- 입출력
  - 백엔드와 프론트엔드 크루 이름을 파일 입출력을 통해 불러옴
  - 기능의 종류를 출력
  - 기능의 번호를 입력받음
    - 올바른 기능의 숫자 혹은 Q가 입력되었는지 검증
  - 기능의 결과를 출력
  - 과정과 미션을 출력
  - 매칭하고자하는 과정, 레벨, 미션을 입력받음
    - 입력받은 값이 "과정, 레벨, 미션"의 형식을 갖는지 검증
    - 입력받은 값이 올바른 과정, 레벨, 미션의 이름인지 검증

- 페어 매칭
  - 크루들의 이름 목록을 랜덤으로 섞음
  - 입력받은 과정, 레벨, 미션의 페어를 매칭
    - 같은 레벨에서 이미 페어로 만난적이 있는 크루끼리 매칭되었는지 검증
      - 이미 페어로 만난 크루가 매칭되었을 경우 페어를 재매칭
      - 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없는지 검증
  
- 페어 조회
  - 입력받은 과정, 레벨, 미션에 해당하는 미션의 페어 정보를 반환
  - 해당 미션의 매칭 이력이 있는지 검증
