...
- [ ] 입력 받는 기능
  - [x] 첫 기능을 입력받는 기능
    - [x] [ERROR} 기능 입력 범위 내의 숫자를 입력한지 검증하는 기능
    - [x] [ERROR] 기능 입력 범위 문자를 입력한지 검증하는 기능
  - [x] File을 읽는 기능.
  - [ ] 과정, 레벨 미션을 입력 받는 기능
  - [ ] 다시 매칭할 지 여부를 입력 받는 기능
...
- [ ] 미션 (Mission)
  ...
  - [ ] 페어를 매칭하는 기능
    ...
    - [x] 이름 목록을 LIST 형태로 준비

    - [x] 크루들의 이름 목록을 랜덤으로 섞는다. (`Randoms.suffle()`)

    - [x] 랜덤으로 섞인 목록에서 앞에서 순서대로 두명씩 페어를 맺는다.

    - [x] 홀수인 경우 마지막 남는 크루는 마지막 페어에 포함 

    - [ ] 같은 레벨에서 이미 페어로 만난적 있는 크루는 다시 페어로 매칭되다면 랜덤으로 섞어서 매칭 재시도

    - [ ] 3회 시도 까지 매칭 되지 않거나 매칭할 수 있는 경우가 있으면 에러 출력



1. 페어 조회
   - [ ] 상응 하는 매칭 결과를 출력하는 기능
   - [ ] [ERROR] 매칭 결과가 없으면 에러를 출력
