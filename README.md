...
1. 기능 선택 번호 입력 요구 출력
2. 기능 선택 번호 입력
- 입력한 기능 선택 번호가 올바른 번호 인지 검증
- 입력한 기능 선택 번호가 없으면 예외 발생
- 입력한 기능 선택 번호에 빈칸이 있으면 예외 발생
- 입력한 기능 선택 번호가 올바른 번호(1, 2, 3, Q) 가 아니면 예외 발생
3. ```1. 페어 매칭``` 선택시 사전 정보 출력
4. 과정, 레벨, 미션 입력 요구 출력
5. 과정, 레벨, 미션 정보 입력
- 입력한 정보가 올바른 정보인지 검증
- 입력한 과정이 ```백엔드``` 또는 ```프론트엔드``` 가 아니면 예외발생
- 입력한 레벨이 ```레벨1``` 또는 ```레벨2``` 또는 ```레벨3```이 아니면 예외발생
- 입력한 미션이 미션 정보에 있는 미션이름과 일치하지 않을 경우 예외 발생
6. ```페어매칭 정보가 있을 경우``` 재매칭 선택 입력 요구 출력
7. ```재매칭``` 선택 입력
- 입력한 재매칭 선택이 올바른지 검증
- 입력한 재매칭 선택이 ```네``` 또는 ```아니오```가 아니면 예외 발생
8. 해당 과정,레벨,미션에 대한 페어매칭 출력
9. ```2. 페어 조회``` 선택시 사전 정보 출력
10. 과정, 레벨, 미션 입력 요구 출력
11. 과정, 레벨, 미션 정보 입력
12. 해당 과정, 레벨, 미션에 대한 페어 출력
13. ```3. 페어 초기화``` 선택시 페어매칭 정보 전체 초기화
14. 초기화 여부 출력
15. ```Q. 종료``` 선택시 프로그램 종료
