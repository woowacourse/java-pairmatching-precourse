...
## 🚀 기능 목록
1. 파일 입력 기능
   1. 프론트엔드, 백엔드 유저 이름 불러오기
   2. 각각 FrontEndManager, BackEndManager에서 Crew 객체로 변환
2. 미션 정보 저장 기능
   1. Course 클래스에 레벨별 과목명 저장
   2. 사전에 주어진 정보를 기반으로 데이터 저장(init())
3. 기능 선택 기능
   1. 과정, 레벨, 미션 순으로 입력
   2. 과정이 백엔드나 프론트엔드가 아니라면 예외 처리
   3. 레벨이 레벨(숫자) 형태가 아니라면 예외 처리
   4. 존재하지 않는 미션인 경우 예외 처리
   5. 종료 입력 시 종료
4. 페어 기능
    1. 매칭 기능
       - 매칭된 두명 혹은 세명이 서로 이전에 만난 적이 있다면 예외처리
    2. 조회 기능
       - 매칭 정보가 존재하지 않는 경우 예외처리
       - 과정, 레벨, 미션 입력받은 후 존재하는 페어 매칭 결과 출력
    3. 초기화 기능
    - 모든 페어 매칭 정보 초기화
