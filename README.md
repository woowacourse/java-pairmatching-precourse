...
## 💻 구현 기능 목록 
Launcher
- 1~3까지 각 페어 매칭, 페어 조회, 페어 초기화 기능이 있다.
- Q를 입력하면 실행이 종료된다.

FileReader
- resources 파일에 있는 Crew원 정보를 읽어온다

Input
- 기능을 입력받는다. (페어 매칭, 조회, 초기화)
- 매칭을 선택할 경우
    - 과정, 레벨, 미션을 입력받는다.
    - 이미 매칭 정보가 있는 경우 갱신 여부를 입력받는다.

Output
-  페어 매칭을 선택할 경우, (과정, 레벨에 따른 미션)을 전부 출력한다.
-  매칭이 이루어질 경우 매칭된 정보를 출력한다.

Matching
- Course, Level, Mission에 따른 매칭 결과를 저장한다.
- 이미 해당 매칭이 등록되어 있는지 여부를 확인한다.
- 매칭 결과를 반환한다.
 
Pair
- 페어 매칭 정보를 저장한다.
    - 2명씩 매칭하고 만약 홀수의 인원이 남을 경우 3명으로 매칭한다.
- 페어 매칭 정보를 조회한다. 

Crew
- Course(백엔드, 프론트엔드)에 따라 분류한다.
- md 파일을 통해 각 크루의 인원을 저장한다.
- 랜덤으로 각 페어를 매칭한다.
    - 같은 레벨에서 이미 페어로 만난적이 있는 크루끼리 다시 매칭될 경우 다시 랜덤으로 섞어서 매칭한다.
    - 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 에러메시지를 출력한다. 
...
Mission
- Level에 따라 0개 또는 그 이상의 Course가 주어진다.
