...
1. 기능 카테고리 질문 기능
2. 카테고리 선택지를 입력 받는 기능
  [] 없는 미션에 대한 예외처리: 입력한 것에 대한 미션이 없는 경우 에러를 발생시키고 다시 물어본다.
   1. 1번이라면 구체적인 내용 질문
   2. 2번이라면 현재 매칭 정보가 있는 지확인 하고 없으면 생성
   3. 3번이라면 있으면 매칭 정보 있다고 알려주고 다시 매칭
4. 해당 카테고리의 구체적인 내용을 출력하는 기능
5. 해당 카테고리에 관련 구체적인 내용을 입력받는 기능
6. 파일을 읽어온다
7. 페어링을 하는 기능
   1. 페어링이 있는 지확인한다
8. 페어링이 제대로 되지 않았을 때 다시 페어링을 하는 기능
9. 3회 시도 후에도 페어링이 되지 않았을 때 에러를 내는 기능 (1번으로 이동한다)
10. 매칭 결과를 출력하는 기능
11. 다시 1번으로 돌아가 질문.
12. Q를 눌러 종료하는 기능
