...
- 초기 설정 (파일 입력)
  - 과정 (백엔드, 프론트엔드)
    - 과정 안에 레벨 (레벨1 ~ 레벨5)
      - 레벨 안에 미션 
  - 크루원
    - 파일을 입력받아 과정으로 크루원 목록을 가져올 수 있도록 구현
...
- 메뉴 출력
  ...
  - 종료 (Q)
- 페어 자료구조 (Map)
  - 과정 : 레벨 : 미션을 key로 사용하여 매칭 현황을 조회할 수 있어야 한다.
  - 저장
  - 조회
  - 페어였던 적 있는지 확인

- 페어 매칭
  - (과정, 레벨, 미션) 형식으로 입력 받는다
  - 정상입력시 매칭 후 결과 출력
    - 매칭 결과는 저장된다.
    - 재매칭
      - 이전에 저장된 결과가 이미 있으면 덮어씌우는지 물어봄
        - 네 -> 매칭 진행
        - 아니오 -> 페어 매칭 처음부터 시작
      - 같은 레벨에서 다른 미션에서 두 크루가 페어인 적이 있으면 다시 매칭
        - 재시도 3회 또는 매칭할 경우의 수가 없으면 에러메시지 출력 후 입력으로 돌아감
          - 크루 인원수가 2명 미만일 때
- 페어 조회
  - (과정, 레벨, 미션) 형식으로 입력받는다
  - 페어 정보 출력
