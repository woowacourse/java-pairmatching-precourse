...
### 필수 요구사항 제약 조건
    ...
    - 같은 레벨에서 이미 페어를 맺은 크루와는 다시 페어로 매칭될 수 없다.
      - 페어 매칭 시다양한 페어를 만나기 위해서 같은 레벨 동안은 같은 페어를 만나지 않는다.
...
### 초기 셋팅 기능
  - 초기 데이터 셋팅 기능
    - 각 레벨 별 미션 셋팅
  - 파일 읽기 기능
    - 직무별 크루 인원 담기(백엔드 프론트엔드)
  

### 입력 기능
  1. 기능 선택 입력 받기 기능
  2. 과정, 레벨, 미션을 입력 받기 기능
     - 양식 := {과정, 레벨, 미션}
  3. 매칭 정보 존재시 재 매칭 여부 입력 받기 기능
...
### 입력 예외 처리 기능 
  ...
  - 과정, 레벨, 미션을 입력 받기 기능
   - 양식과 일치하지 않는 경우
  - 매칭 정보 존재시 재 매칭 여부 입력 받기 기능
    - '네 or 아니오' 가 아닌 경우

### 페어 매칭 기능
  - 각 크루 별 매칭 기록 저장 기능 Map<{단계},{Set<크루>}>>
  - 각 크루 별 매칭 기록 확인 기능 map.get(단계).contains(크루) := true(매칭된 기록 있음) , false(매칭된 기록 없음)
  - 크루들 랜덤으로 섞기 기능(shuffle)
  - ```
    ### 페어 매칭 로직
    1. 크루들의 모든 이름을 List<String>에 넣는다
    2. 크루 목록을 섞기 (camp.nextstep.edu.missionutils.Randoms의 shuffle 기능 필수 사용)
    3. 매칭은 앞에서 부터 순서대로 두명씩 페어를 맺기
      - 각 크루 별 같은 레벨에서 만난적 여부 판단
      - 만남 여부
        - true := 크루 목록 다시 섞기(shuffle 필수 사용)
        - false := 진행. 
    ```

### 페어 조회 기능
  - 이전 매칭 기록이 없다면 다시 기능 선택 입력으로 돌아가기
  - 매칭 기록이 있다면 매칭 기록 보여주기

### 초기화 기능
  - 매칭 기록 초기화 진행
  - 매칭 기록은 초기화 되지만 각 크루들의 속성중 Map은 제거되지 않음.

#### 만약, 3회 시도까지 매칭이 되지 않거나 또는 매칭을 할 수 있는 경우의 수가 존재하지 않는다면 에러메시지를 출력한다.
