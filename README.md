...
## 기능 구현
- Crew 정보 파일에서 읽어오기
  - Crews에서 파일로 사용자 정보를 읽어오고 Crew에 하나씩 넣은 후, Crews 속에 백엔드, 프론트엔드 사람 리스트를 만들어 낸다.
- 레벨별로 담고있는 미션들을 저장한다.
  - 각 레벨에 이름을 넣고, 그 안에 미션들을 리스트로 저장한다. 미션이 없는 경우 리스트는 만들어져만 있고 비어있게 된다. 
  - 이미 주어진 값들로 사용해야하므로 직접 넣어 하드코딩한다. (상수로 하는 것이 큰 의미 없어보임)
- 코스별로 백엔드, 프론트엔드 enum을 만들고 그 값에 따라 해당하는 crews값을 가져올 수 있게 한다.
- 게임이 실행될 수 있는 메뉴 선택 부분을 구현한다.(메인 구현 부분)
- 페어 매칭 실행
  - 과정, 레벨, 미션을 입력받는데 없는 과정, 레벨, 미션의 경우 에러를 발생시킨 후 다시 입력 받는다. ,로 구분한다.(,이외의 문자가 포함된 경우 에러처리 한다.)
  - 페어매칭 되는 사람들을 key값으로 하고, 다음 서로 매칭된 사람들을 저장한다. (해시맵 사용, 이때 0번째 인덱스는 백엔드, 프론트엔드로 구분하기 위함으로 사용하고 실 사용자는 1번부터 저장한다.)
  - 해당 해시맵을 각 레벨별에 미션 리스트에 리스트를 만들어서 저장한다.
  - 매칭이 되어 있다면 매칭 정보가 있는데 다시 매칭할 것 인지를 물어본다.
  - 만약 이미 만난 적이 있는 크루가 있다면 해당 과정을 다시 진행한다.
  - 3회까지 매칭이 되지 않으면 에러메세지를 출력한 후 메뉴 선택 부분으로 돌아간다.
- 페어 조회
  - 과정, 레벨, 미션을 입력받는데 없는 과정, 레벨, 미션의 경우 에러를 발생시킨 후 다시 입력 받는다. ,로 구분한다.(,이외의 문자가 포함된 경우 에러처리 한다.)
  - 있다면 보여주고 없다면 없다고 말한다.
- 페어 초기화
  - 모든 페어를 초기화 한다.
- 종료
  - 페어 프로그램을 모두 종료한다.
