...
## 기능 목록
- 프로그램 시작시 기능 선택 기능
  - Q.종료를 누르기 전까지 계속 반복한다.
  - 각 기능을 선택하면 그에 맞는 기능을 호출한다.
- 과정, 레벨, 미션 출력 기능
  - 사용자에게 어떤 과정과 레벨이 있는지 출력해주는 기능
- 과정, 레벨, 미션 선택 기능
...
- 페어 매칭 기능
  - 페어는 두명 씩 매칭한다
  - 크루 이름을 파일에서 가져오는 기능
  - 페어 매칭 대상의 총 수가 홀수인 경우 맨마지막 그룹은 3명으로 매칭한다.
  - 같은 레벨에서 이미 페어를 맺은 크루가 있을 경우 다시 매칭을 시도한다.
  - 3회 시도까지 매칭에 실패한 경우 에러 메세지를 출력한다.
- 페어 매칭 저장 기능
  - 페어 매칭 기능으로 생성된 매칭 결과는 저장되어야한다.
- 페어 조회 기능
  - 사용자가 2를 입력하면 과정, 레벨, 미션 출력 기능을 호출한다.
  - 사용자가 과정, 레벨, 미션을 입력하면 저장되어 있는 페어 매칭을 결과를 호출한다.
- 페어 초기화 기능
  - 사용자가 3을 입력하면 저장되어 있는 페어 정보를 모두 초기화한다.
- 종료 기능
  - 사용자가 Q를 입력하면 프로그램을 종료한다. 
