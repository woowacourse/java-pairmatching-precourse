...
- [ ] 사용자는 페어 매칭, 페어 조회, 페어 초기화, 종료의 기능을 선택할 수 있다.
  - [ ] 해당 기능 외의 입력을 받으면 에러 메시지를 띄운다.
- [ ] 페어 매칭을 선택하면 매칭할 과정, 레벨, 미션을 입력할 수 있다.
  - [ ] 과정, 레벨, 미션은 이미 정해진 값 내에서 입력해야 한다.
- [x] 백앤드 혹은 프론트앤드 인원을 자바 입출력으로 불러온다.
- [x] 이름 목록을 랜덤으로 섞는다.
- [ ] 앞에서 부터 2명씩 묶은 후 인원이 홀수여서 1명이 남는다면 마지막 팀에 포함 시킨다.
- [ ] 같은 레벨에서 이미 페어로 만난 적이 있는 크루끼리 매칭되면 다시 진행한다. (이전 결과와 비교)
- [ ] 3회 시도까지 매칭이 되지 않거나 경우의 수가 없으면 에러를 출력한다.
- [ ] 매칭이 된 결과는 저장이 되어야 한다.
