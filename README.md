...
## 구현 기능 목록
- md파일로 저장된 크루들을 List<Class>로 만든다. (o)
- 페어들을 매칭한다.
  - 과정, 레벨, 미션을 입력받는다. (o)
    - 과정이 없으면 예외를 반환한다. (o)
    - 레벨이 없으면 예외를 반환한다. (o)
    - 해당 레벨에 입력받은 미션이 없으면 예외를 반환한다. (o)
    - 매칭 정보가 있을 경우 예를 입력하면 새로 매칭을 한다. (o)
    - 매칭 정보가 있을 경우 아니오를 입력하면 그대로 진행한다. (o)
  - 페어 쌍을 만든다. (o)
    - 홀수인 경우 마지막 페어는 3명으로 구성된다. (o)
    - 들어온 두 크루가 같은 레벨에서 페어가 된 적 있으면 다시 만든다. (o)
    - 3회 시도까지 페어가 만들어지지 않으면 or 매칭할 수 있는 경우의 수가 없으면 예외를 반환한다. (o)
  - 페어 쌍들을 조회한다. (o)
    - 과정, 레벨, 미션을 입력받는다. (o)
      - 과정이 없으면 예외를 반환한다. (o)
      - 레벨이 없으면 예외를 반환한다. (o)
      - 해당 레벨에 입력받은 미션이 없으면 예외를 반환한다. (o)
    - 해당 정보에 페어가 존재하지 않으면 예외를 반환한다. (o)
  - 전체 페어 쌍들을 초기화한다. (o)
<br>
- 도메인
  - 코스
