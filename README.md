...
- UI 기능
  - 기능 선택을 요청하는 문구 출력
  - 어떤 기능을 선택할지 숫자로 입력받는 기능
  - 페어 매칭 기능을 선택했을 시, 과정과 미션을 출력
  - 쉼표로 구분하여 과정, 레벨, 미션을 입력받는다.
  - 매칭이 완료되면 결과를 출력하는 기능.
  - 이미 매칭된 적이 있다면 다시 매칭을 제시하는 문구 출력
  - 네/아니오 를 기준으로 재매칭을 할 것인지 입력받는 기능
  - 초기화를 선택했을 시 초기화 되었다는 문구 출력
  - 에러가 났을 씨, 에러 문구를 전달받아 출력하는 기능 
  
- 크루
  - 과정에 따라 프론트/백 중에서 올바른 크루들의 이름을 불러오는 기능
  - 크루 이름을 파일에서 불러와 섞어주는 기능
  
- 레벨(미션)
  - 선택한 레벨의 미션을 세팅하는 기능.
  
- 코스
  - 입력받은 코스명을 검증하고 해당 코스의 객체를 불러와서 크루에 등록한다.
...
- 페어매칭 시스템
  ...
  - 페어 초기화 기능
  - 종료 기능
    - 모든 기능은 크루 리스트의 생성(매칭)과 출력(조회)를 기반으로 한다. 

- 검증
  - 입력받은 기능 선택이 1,2,3,Q 중에 있는지 검증하고 없다면 예외처리
  - 입력받은 과정/레벨/미션 선택이 쉼표를 기준으로 하는지 검증하고 정해진 과정과 레벨과 미션에 있는 내용인지 확인
  - 네/아니오 로 받은 입력이 스페이스바 등 정확히 일치하는 지 검증.
