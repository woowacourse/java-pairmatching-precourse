...
### 기능 선택
 - [x] 프로그램의 기능 종류를 출력 하고 그 중 하나를 입력 받는다.
   - [ ] 입력이 1 혹은 2 혹은 3 혹은 Q인지 검증한다.
   - [ ] 검증에 실패하면 `[ERROR] 제공된 기능 중 하나를 선택 하세요`을 출력하고 다시 입력 받는다.

### 페어 매칭
 - [ ] 우테코에서 제공되는 과정, 레벨, 미션을 출력하고 이 셋을 입력 받는다.
   - [ ] 각 정보는 `,[공백]`으로 구분된다.
   - [ ] 과정, 레벨, 미션 3개의 정보가 입력되었는지 검증한다.
   - [ ] 검증에 실패하면 `[ERROR] 과정, 레벨, 미션을 입력하세요`를 출력하고 다시 입력 받는다.
