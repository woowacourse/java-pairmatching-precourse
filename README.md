...
- [ ] 페어 매칭을 선택하면 매칭할 과정, 레벨, 미션을 입력할 수 있다.
  - [ ] 과정, 레벨, 미션은 이미 정해진 값 내에서 입력해야 한다.
...
- [ ] 3회 시도까지 매칭이 되지 않거나 경우의 수가 없으면 에러를 출력한다.
- [x] 입력 받는 과정, 레벨, 미션이 있는지 확인한다.
- [ ] 이미 해당 과정, 레벨, 미션이 있는데 다시 매칭하고자 하면 매칭 정보가 존재한다는 문구를 띄우고 재매칭 여부를 입력 받는다.
- [ ] 기존 매칭결과를 재매칭 결과로 대체한다.
- [ ] 페어 조회를 선택하면 조회할 과정, 레벨, 미션을 입력할 수 있다.
- [ ] 해당 입력으로 조회된 페어가 없는 경우 에러 메시지를 띄운다.
- [ ] 해당 입력으로 조회된 페어가 있으면 제일 최근에 매칭된 페어를 띄운다.
- [ ] 페어 초기화를 선택하면 모든 페어를 초기화 한다.
