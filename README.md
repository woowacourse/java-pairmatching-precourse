...
## 🔨 기능 목록
> 우테코 미션을 함께할 페어를 관리해주는 애플리케이션 제작
- 같은 레벨 동안은 같은 페어를 만나지 않는다.

#### 시스템 준비
- 백엔드, 프론트엔드 크루들의 이름 목록 파일을 읽는다.
  - 예외 : 파일 읽기에 실패한 경우
...
- 기능의 종류를 출력한다.
- 사용자는 기능 중 하나를 선택한다.
  - 예외 : 기능(1,2,3,Q) 외의 문자를 입력한 경우

#### 1. 페어 매칭
- 과정과 미션을 출력한다.
- 사용자는 과정, 레벨, 미션을 입력한다.
  - 예외 : 예시와 다른 형식으로 입력 한 경우 
  - 예외 : 없는 미션인 경우

- 매칭 정보가 이미 존재할 경우 안내문을 출력한다. `구현하지 못했습니다ㅠㅠ`
- 사용자는 네, 아니오 중 하나를 입력한다.
  - 예외 : 네, 아니오 외의 다른 문자를 입력한 경우

- 크루 목록을 랜덤 셔플한다.
- 앞에서부터 순차적으로 2명씩 페어를 맺는다.
- 끝에서 1명이 남을 경우 마지막 페어에 포함시킨다.
- 이때 같은 레벨에서 이미 만난적이 있는 크루끼리 매칭 될 경우 랜덤 셔플부터 다시 반복한다. `구현하지 못했습니다ㅠㅠ`
- 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 에러 메시지를 출력한다. `구현하지 못했습니다ㅠㅠ`
- 페어 매칭 결과를 출력한다.
  - shuffle 메서드의 결과 순서로 정렬한다.
...
- 메서드의 길이가 15라인을 넘기지 않는다. (한 가지 일만 한다!!)
- else 예약어를 쓰지 않는다. (if 조건절에서 값을 return 하는 방식으로 구현한다!!)
- build.gradle 파일을 변경할 수 없고, 외부 라이브러리를 사용하지 않는다.
- 프로그램 종료 시 System.exit()를 호출하지 않는다. 
- JDK8에서 모든 테스트 케이스를 통과해야한다. (./gradlew clean test)
